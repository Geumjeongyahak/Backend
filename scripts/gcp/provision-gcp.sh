...
ENV_FILE="${1:?usage: scripts/gcp/provision-gcp.sh scripts/gcp/env/prod.env}"
...
STARTUP_TEMPLATE="${ROOT_DIR}/scripts/gcp/startup-docker.sh"
STARTUP_RENDERED="$(mktemp)"
...
if ! gcloud iam service-accounts describe "${APP_SERVICE_ACCOUNT_EMAIL}" --project="${PROJECT_ID}" >/dev/null 2>&1; then
  gcloud iam service-accounts create "${APP_SERVICE_ACCOUNT_NAME}" \
    --project="${PROJECT_ID}" \
    --display-name="Sonmoum ${ENVIRONMENT:-env} app service account"
fi
...
ensure_firewall "sonmoum-${ENVIRONMENT}-allow-http" \
  --allow="${HTTP_PORTS:-tcp:8080}" \
  --source-ranges="${HTTP_SOURCE_RANGES:-0.0.0.0/0}" \
  --target-tags="${APP_NETWORK_TAG}"

ensure_firewall "sonmoum-${ENVIRONMENT}-allow-app-observability" \
  --allow=tcp:9090,tcp:9100 \
  --source-ranges="${MONITORING_SOURCE_RANGES:-10.0.0.0/8}" \
  --target-tags="${APP_NETWORK_TAG}"

if [[ "${SKIP_DB_INSTANCE:-false}" != "true" ]]; then
  ensure_firewall "sonmoum-${ENVIRONMENT}-allow-db-observability" \
    --allow=tcp:9100 \
    --source-ranges="${MONITORING_SOURCE_RANGES:-10.0.0.0/8}" \
    --target-tags="${DB_NETWORK_TAG}"

  ensure_firewall "sonmoum-${ENVIRONMENT}-allow-app-to-postgres" \
    --allow=tcp:5432 \
    --source-tags="${APP_NETWORK_TAG}" \
    --target-tags="${DB_NETWORK_TAG}"
fi

ensure_firewall "sonmoum-${ENVIRONMENT}-allow-ssh-app" \
  --allow=tcp:22 \
  --source-ranges="${SSH_SOURCE_RANGES:-0.0.0.0/0}" \
  --target-tags="${APP_NETWORK_TAG}"

if [[ "${SKIP_DB_INSTANCE:-false}" != "true" ]]; then
  ensure_firewall "sonmoum-${ENVIRONMENT}-allow-ssh-db" \
    --allow=tcp:22 \
    --source-ranges="${SSH_SOURCE_RANGES:-0.0.0.0/0}" \
    --target-tags="${DB_NETWORK_TAG}"
fi

ensure_firewall "sonmoum-${ENVIRONMENT}-allow-iap-ssh" \
  --allow=tcp:22 \
  --source-ranges=35.235.240.0/20 \
  --target-tags="${APP_NETWORK_TAG}"

if [[ "${SKIP_DB_INSTANCE:-false}" != "true" ]]; then
  ensure_firewall "sonmoum-${ENVIRONMENT}-allow-iap-ssh-db" \
    --allow=tcp:22 \
    --source-ranges=35.235.240.0/20 \
    --target-tags="${DB_NETWORK_TAG}"
fi

echo "[6/8] Render startup script"
{
...
echo "[8/8] Outputs"
"${ROOT_DIR}/scripts/gcp/print-outputs.sh" "${ENV_FILE}"
