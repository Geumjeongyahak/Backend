...
public class DepartmentParticipateService {
    ...
    public void joinDepartment(Long userId, Long departmentId) {
        ...
        var department = departmentRepository.findById(departmentId)
                .orElseThrow(() -> new ResourceNotFoundException(ErrorCode.RESOURCE_NOT_FOUND, departmentId));

        // 중복 체크
        if (userDepartmentRepository.existsByUserIdAndDepartmentId(userId, departmentId)) {
            throw new DuplicateResourceException(ErrorCode.DUPLICATE_RESOURCE, "이미 해당 부서에 참여한 사용자입니다.");
        }
        // 참여 처리
    ...
    public void leaveDepartment(Long userId, Long departmentId) {
        ...
        if (!userDepartmentRepository.existsByUserIdAndDepartmentId(userId, departmentId)) {
            throw new ResourceNotFoundException(ErrorCode.RESOURCE_NOT_FOUND, "해당 부서에 참여하지 않은 사용자입니다.");
        }
