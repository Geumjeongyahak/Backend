...
| 보조 도메인 필요 여부 | DailySchedule 기반(AbsenceRequest, LessonExchangeRequest), 분반 또는 부서 기반(PurchaseRequest) |
...
| 과목/수업/DailySchedule 기반 요청 | `TestLessonHelper`로 테스트별 독립 수업 생성 후 필요한 DailySchedule 조회 |
...
```java
@Test
@DisplayName("[Side-effect] 결석 요청 승인 → DailySchedule 교사 출석 공결 반영")
void approve_updatesDailyScheduleTeacherAttendanceToExcused() {
    ...
    DailyTeacherAttendance attendance = dailyTeacherAttendanceRepository
        ...
        .orElseThrow();
    assertThat(attendance.getStatus()).isEqualTo(DailyTeacherAttendanceStatus.EXCUSED);
}
```
...
| 트리거 | 변경 엔티티 | 변경 필드 |
|--------|-----------|---------|
| AbsenceRequest 승인 | AbsenceRequest | `status` → `APPROVED`, 승인자/승인 시각 기록 |
| AbsenceRequest 승인 | DailyTeacherAttendance | `status` → `EXCUSED` |
