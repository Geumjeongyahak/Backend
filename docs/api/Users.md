...
### 1.7. 사용자 계정 비활성화
사용자 레코드와 운영 이력을 보존하면서 계정 접근을 차단합니다.
...
- **처리 방식**:
  - `users.is_deleted=true`, `deleted_at=처리 시각`으로 변경합니다.
  - 사용자 자격 증명과 직접 권한은 삭제하지 않습니다.
  - 발급된 Refresh Token을 모두 즉시 폐기합니다.
  - 활성 Push 구독을 모두 즉시 해제합니다.
  - 기존 Access Token, 로컬/관리자/Google 로그인 및 토큰 재발급을 차단합니다.
  - 사용자 상세/목록, 교사 후보, 연락망 및 운영 집계에서 제외합니다.
  - 게시글, 댓글, 요청, 수업 등 기존 이력의 사용자 ID와 이름은 유지합니다.
- **제약 사항**:
  - 본인 계정은 비활성화할 수 없습니다.
  - 마지막 활성 관리자 계정은 비활성화할 수 없습니다.
  - 담당 중인 활성 과목이 있으면 `409 Conflict`를 반환합니다.
  - `PENDING` 교원 신청·결석 요청이 있으면 `409 Conflict`를 반환합니다.
  - `PENDING` 또는 `APPROVED` 구입 요청이 있으면 `409 Conflict`를 반환합니다.
  - `PENDING` 또는 `APPROVED` 수업 교환 요청이나 `ACTIVE` 수업 교환 제안이 있으면 `409 Conflict`를 반환합니다.
  - 이미 비활성화된 사용자는 `404 Not Found`로 처리합니다.
  - 비활성 계정의 이메일은 재가입에 사용할 수 없습니다.
